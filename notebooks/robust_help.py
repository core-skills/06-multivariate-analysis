...
def _initialise_randomstate():
    """ Use RandomState object with seed set to 7."""
    return np.random.RandomState(7)

def _draw_sulphur_observations(count):
    rs = _initialise_randomstate()
...
def _draw_dilithium_observations(count):
    rs = _initialise_randomstate()
    return  rs.uniform(10, 25, count)

def _draw_kryptonite_observations(count):
    rs = _initialise_randomstate()
    return rs.uniform(5, 15, count)
...
def create_templates_matrix():
    ...
    # a column of ones.
    x_axis = MultichannelXAxis(5, 90, 0.02)
    template_count = 4
    ...
    # dilithium is very similar to quartz, except for the last reflection
    dilithium_reflections = []
    dilithium_reflections.append (Reflection(intensity=10.0, two_theta=25.0, broadening=3.0))
    dilithium_reflections.append (Reflection(13.0, 38.0, 6.0))
    dilithium_reflections.append (Reflection(10.0, 43.0, 12.0))
    dilithium_reflections.append (Reflection(25.0, 70, 2.0))
    ...
    unobtainium_reflections.append (Reflection(1.0, 23.0, 12.0))
    unobtainium_reflections.append (Reflection(5.0, 31.0, 2.0))
    unobtainium_reflections.append (Reflection(3.0, 55.0, 6.0))
    unobtainium_reflections.append (Reflection(7.0, 58.0, 12.0))
...
def create_composition_dataframe(observations_count):
    ...
    Returns:
        ...
        These values are drawn from uniform distributions that add
        up to 100."""
    unobtainium_percent = _draw_unobtainium_observations (observations_count)
    dilithium_percent = _draw_dilithium_observations(observations_count)
    kryptonite_percent = _draw_kryptonite_observations(observations_count)
    quartz_percent = 100 - unobtainium_percent - \
    kryptonite_percent - dilithium_percent

    return pd.DataFrame({'Quartz': quartz_percent,
                        'Dilithium': dilithium_percent,
                        'Kryptonite': kryptonite_percent,
                        'Unobtainium': unobtainium_percent})
...
def create_observations(compositions_dataframe, templates):
    ...
    for observation_idx in range (0, observations_count):
        ...
        # add gaussian noise with mean 20 and standard deviation 5 units
        observations_matrix[:, observation_idx] = \
            add_gaussian_noise(observations_matrix[:, observation_idx], 25, 10)
